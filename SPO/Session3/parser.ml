...
let nts_of_production pr =
  let add_symbol acc = function
    | NonTerminal s -> Ntset.add s acc
    | _ -> acc
  in List.fold_left add_symbol Ntset.empty pr
let non_terminals (g : grammar) =
  let add_rule acc (ns, pr) = Ntset.union (nts_of_production pr) (Ntset.add ns acc)
  in List.fold_left (add_rule) Ntset.empty g.rules
...
let rec fixpoint (f : 'a -> 'a * bool) (x : 'a) =
  assert false (* TODO *)

(* Test of fixpoint : *)
let factorial m =
  let fact_aux (a,b) =
    if b <= 0 then ((a,b), false) else ((a*b, b-1), true) in
...
(* Write a function val empty_map: grammar -> Tset.t Ntmap.t which constructs
   ...
   of terminals. *)
let empty_map (g: grammar) : Tset.t Ntmap.t =
  assert false (* TODO *)
...
let rec first_production_step (nls : nulls) (fsts : firsts) (pr: production) : Tset.t =
  assert false (* TODO *)
...
let first (g : grammar) (nls : nulls) : firsts =
    assert false (* TODO *)
...
let expansions (g: grammar) : expansion_table =
  assert false (* TODO *)
...
let is_ll1 (table : expansion_table) : bool =
    assert false (* TODO *)
 ...
 *)
let analyze (nt : non_terminal) (t : expansion_table) (word : string list) : bool =
   assert false (* TODO *)
